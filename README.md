...
## 📸 프로젝트 흐름 및 이미지 기록
> 관리자 PC, 사용자 PC, 키오스크가 서로 따로 보이지 않도록 "전체 순서도 → 체크섬 통신 → 운영 화면 → 주문/매출 화면" 순서로 정리했습니다.

| 화면 | 설명 |
|------|------|
| ![시스템 순서도](images/01.png) | 시스템 순서도 — 사용자 PC·키오스크·관리자 PC 프로그램의 동작 흐름과 TCP 메시지 전달 구조 |
| ![키오스크 충전 화면](images/02.jpg) | 키오스크 충전 이벤트 실행 화면 — 결제 승인 후 서버로 충전 요청을 보내고 사용자 PC 잔여시간과 동기화 |
| ![관리자 매출 차트](images/03.jpg) | 관리자 매출 확인 화면 — 시간대별 매출 막대 차트를 조회하고 DB 반영 결과를 확인 |
| ![체크섬 프로토콜](images/04_checksum-protocol.png) | 체크섬 Boxing/Unboxing 개념 — UTF-8 한글 메시지 전송 중 데이터 손상을 가볍게 검증하기 위해 사용한 패킷 무결성 구조 |
| ![관리자와 사용자 로그인](images/05_admin-user-login-flow.png) | 관리자 PC와 사용자 PC/키오스크 로그인 흐름 — 좌석 상태와 회원가입/로그인 화면을 함께 보여주는 초기 진입 화면 |
| ![운영 중 좌석 동기화](images/06_admin-user-runtime.png) | 운영 중 좌석 동기화 — 관리자 PC의 50석 현황과 사용자 PC 상태, 메시지 로그가 동시에 갱신되는 장면 |
| ![키오스크 주문 흐름](images/07_kiosk-order-flow.png) | 키오스크 주문 흐름 — 상품 선택, 장바구니, 주문 완료 알림, 관리자 로그 갱신이 이어지는 시나리오 |
| ![관리자 상품/주문 관리](images/08_admin-product-sales.png) | 관리자 상품·주문 관리 — 상품 재고/판매/주문 목록을 조회·수정·삭제하는 운영 화면 |
| ![키오스크 주문 완료](images/09_kiosk-order-complete.png) | 주문 완료 팝업 — 주문번호와 총액을 표시하고 관리자 DB 갱신으로 이어지는 마지막 단계 |
